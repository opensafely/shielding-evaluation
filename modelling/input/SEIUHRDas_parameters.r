...
pars <- within(pars, {
  #TODO: Re-weight u y h again with latest ons
    ...
    pscomix = sum(psagecomix*ageons) #0.078950 #7.9%, while 8.7% in CoMix prior truncation etc
    ...
    pscoh    = sum(psagecoh*pars$ageons)   #0.043025 #4.3% as overall JDat13.
    ...
    fu     <- 0.5 #0.7        #relative transmission of U group, Davies Lancet ID
    ...
    k      <- 1#5#1           #dispersion/shape parameter of NB likelihood (and noise if simulating). NB also used in Davies 2020 Nat Med
