...
pars <- within(pars, {
  #TODO: weigh u y h m with tpp
    ...
    agetpp <- c(0.043 , 0.082,  0.066,  0.152,  0.141,  0.129,  0.136,  0.106,  0.134);   agetpp=agetpp/sum(agetpp)  #tpp; missing data: 0.01
    u_mean <- mean(u)
    ...
    Na0    <- agetpp*Npop #ageons*Npop     #Initial population size by age group 
    ...
    k      <- 1#200           #dispersion/shape parameter of NB likelihood (and noise if simulating), Davies 2020 Nat Med
    cmdim1 <- na
    cmdim2 <- na
    cmdim3 <- 52
