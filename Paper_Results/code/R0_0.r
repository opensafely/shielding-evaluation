...
R0_0 <- function(pars,GetBeta=1,GetOutput=0,Sampling=0, nt=pars$cmdim3 ){
...
## NGM MEV
  ...
  for (i in 1:ntcontact) { #1:52
    for (k in 1:9){ 
      ...
      for (j in 1:9) {
            ngm[k,j,i] = beta0*u[j]*cm_0[k,j,i]*( y_k*( (1+fuO*(rIO*d_k*orOD))/( rIR*(1-h_k-d_k) + rIH*(h_k) + rIO*(d_k) )) + fu*(1-y_k)*orUR) }}
...
## want new pars$R0
  ...
  if(Sampling==1){
  #R0 over time based on estimated R0 at week 1 & trend of NGM max eigenvalue
  R0_week = pars$R0 * R0_week/R0_week[1] }
...
} else { #if nt<52, up to "2020-12-28"
  Date = c(as.Date(paste(Week[1:ntcontact], "2020", 'Mon'), '%U %Y %a')) }
WeekDateR0 <- tibble(Week=Week, Date=Date, R0_week=R0_week, R0xcmMEV=pars$R0*cmMEV/cmMEV[1],)

if(Sampling==0){ ######################################################################
#csv
  write.csv(WeekDateR0, row.names = F, file = paste0(output_dir,"/",pset$File_R0_week))
#pdf
  colors <- c("R0-cm" = "green", "R0-NGM"  = "darkblue")
  p  <- ggplot(WeekDateR0, aes(x = Date)) +
    geom_line (aes(x=Date, y = R0xcmMEV, color = "R0-cm")) +
    geom_line (aes(x=Date, y = R0_week,  color = "R0-NGM")) +
    labs(x = 'Date', y = 'R0 (given input parameters)', color = "Legend") +
    scale_color_manual(values = colors, breaks=c("R0-NGM", "R0-cm")) + 
	theme(  legend.title=element_blank(), 
	        legend.position = "inside", 
	        legend.position.inside = c(0.25, 0.95))  #x, y
  pdf(paste0(output_dir,"/",pset$File_R0_week,".pdf")); print(p); invisible(dev.off())
} #Sampling==0 ########################################################################

r0<-list(pars,WeekDateR0); 
r0<-setNames(r0,c("pars","WeekDateR0"))

} else if(GetOutput==0)  {
  
r0<-list(pars); 
r0<-setNames(r0,c("pars"))
